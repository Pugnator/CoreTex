...
void ov528::wait_reply(uint32_t expected)
{
  WAIT_FOR(500);
  do
  {
...
{
  uint8_t wake[] = {0xaa, 0x0d, 0, 0, 0, 0};
  uint8_t ack[] = {0xaa, 0x0e, 0x0d, 0, 0, 0};
...
{
  uint8_t cmd[] = {0xaa, 0x1, 0, 0x7, 3, 0x7};

  uint8_t ack[] = {0xaa, 0x0e, 0x01};
  docmd(cmd);
  wait_reply();
...
bool ov528::snapshot(void)
{
  ...
  uint8_t cmd[] = {0xaa, 0x5, 0, 0, 0, 0};

  uint8_t ack[] = {0xaa, 0x0e, 0x05};
  docmd(cmd);
  wait_reply();
...
void ov528::start_transfer(void)
{
  ...
  uint32_t blocksn = expected_pic_size % 1024 ? expected_pic_size / 1024 + 1 : expected_pic_size / 1024;
  ...
  docmd(stop);
  pictransfer = false;
  DEBUG_LOG(0, "Transfer completed, %u == %u\r\n", pic_size, expected_pic_size);
  disk.f_close(&pic);
}

uint8_t *ov528::next_block(uint32_t *size)
{
