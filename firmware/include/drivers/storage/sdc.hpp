...
public:
 uint8_t detect(void);
 SD_Error initialize(void);
 SD_Error read_block(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize);
 SD_Error write_block(const uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize);
 SD_Error get_response(SD_Error response, SD_RESPONSE_TYPE type = SDRT_1);
 SD_Error get_CID(SD_CID* SD_cid);
 SD_Error get_CSD(SD_CSD* SD_csd);

 uint8_t get_data_response(void);
 void sdc_cmd(uint8_t cmd, uint32_t arg, uint8_t crc);
