...
{ "OK", "CONNECT", "RING", "NO CARRIER", "ERROR",
  "", //Any response from SIM900
  "NO DIALTONE", "BUSY", "NO ANSWER", "Call Ready", "NORMAL POWER DOWN", "",
  ">", "READY" };
...
const char *
ATMODEM::get_cmd_str (CMD::ATCMD cmd)
{
 return ATCMD_TEXT[cmd];
}

void
ATMODEM::isr (word address)
{
...
void
ATMODEM::rawcmd (CMD::ATCMD cmd, CMDMODE::MODE mode, const char* arg)
{
 reset ();
 if ( CMD::AT != cmd )
 {
...
void
ATMODEM::rawcmd (CMD::ATCMD cmd, CMDMODE::MODE mode, word arg)
{
 //XXX: fix it
 char result[2] =
 { 0 };
...
bool
ATMODEM::wait_for_reply (CMD::ATCMD cmd, ATRESPONSE expected, word timeout)
{
 ok = false;
 char buf[MODEM_IN_BUFFER_SIZE + 1] =
 { 0 };
