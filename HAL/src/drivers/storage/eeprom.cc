...
void
Eeprom::write (uint16_t cell, uint8_t value)
{
 ...
 {
  *(reinterpret_cast<volatile uint16_t*> (EEPROM_DATA + i)) = (backup[i + 1]
    << 8) | (backup[i] & 0xff);
 ...
 FLASH->CR &= ~FLASH_CR_PG;
 lock ();
 FREE(backup);DEBUG_LOG(0, "Result: 0x%X\r\n", *((uint8_t*)EEPROM_DATA+cell));
}

uint8_t
Eeprom::read (uint16_t cell)
{
 if ( EEPROM_SIZE * 1024 <= cell )
 {
  DEBUG_LOG(0, "requested EEPROM cell is out of bounds [%u > %u]\r\n", cell, EEPROM_SIZE * 1024);
