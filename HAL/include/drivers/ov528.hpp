...
public:
 ov528 (short ch, FATdisk& _disk) :
   USART::USART (ch, 115200, this), imageblk_size (0), imageblk (nullptr), pictransfer (
     false), buf_size (0), pic_size (0), expected_pic_size (0), disk (_disk)
 {
  self = this;
 ...
 soft_reset (void);
 bool
 request_picture (void);
 void
 start_transfer (void);
 bool
 default_setup (void);
 uint8_t*
 next_block (word *size);
 word
 get_block_size ();
