...
word
write_word (word val, uint8_t *buf)
{
 word ci = 0;
 if ( val == 0 )
 {
...
uint16_t
xmodem::crc16 (uint8_t *buf, word size)
{
 uint16_t crc = 0;
...
bool
xmodem::block_tx (uint8_t *data, word blockn)
{
 txmode = true;
 ack = false;
 nack = false;
...
bool
xmodem::send_header (char *filename, word filesize, word blockn)
{
 txmode = true;
 ack = false;
 nack = false;
...
void
xmodem::send_data (uint8_t *data, word size)
{
 wait ();
 uint8_t bn = 1;
 word curp = 0;
