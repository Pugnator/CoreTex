...
Spi::Spi (char ch)
{
 ...
 switch (channel)
 {
  case 1:
   ...
   /* enable all clock */
   PORT_ENABLE_CLOCK_ENTRY(SPI1SCK_PIN)PORT_ENABLE_CLOCK_ENTRY(SPI1MOSI_PIN)
...
void
Spi::multiread (uint8_t *buf, uint32_t size)
{
 if ( 1 == size )
 {
  go8bit ();
...
void
Spi::multiwrite (const uint8_t *buf, uint32_t size)
{
 if ( 1 >= size || size % 2 )
  return;
